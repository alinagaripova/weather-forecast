...
function rebuildWeatherData(weatherList, city, description1, description2, description3) {          //добавление данных сервера в шаблон
    ...
    const date = new Date(weatherList.list[0].dt * 1000);                   //добавление даты
    const day1 = date.getDate();
    date1El.innerHTML = `
    Сегодня, ${date.toLocaleString("ru", {year: 'numeric', month: 'long', day: 'numeric'})}
    `;

    const date2 = new Date(weatherList.list[1].dt * 1000);
    const day2 = date2.getDate();
    date2EL.innerHTML = `
    Завтра, ${date2.toLocaleString("ru", {year: 'numeric', month: 'long', day: 'numeric'})}
    `;

    const date3 = new Date(weatherList.list[2].dt * 1000);
    const day3 = date3.getDate();
    date3EL.innerHTML = `
    Послезавтра, ${date3.toLocaleString("ru", {year: 'numeric', month: 'long', day: 'numeric'})}
    `;

    sDate1El.innerHTML = `  
    ${day1} апреля
    `;                                      //добавление короткой даты
    sDate2EL.innerHTML = `
    ${day2} апреля
    `;
    sDate3EL.innerHTML = `
